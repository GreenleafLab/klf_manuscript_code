...
def compute_hyperparameters():
    ...
    '''
    # update methylation probabilities for short methylation time data
    p_unmeth_given_bound = 0.99 # 0.9
    p_unmeth_given_unbound = 0.4 # 0.1

    # t is the thing we measure as a 1 in the data (i.e. unmethylated = bound = protected)
    p_t_given_meth = 0.01
...
def augment_methyl_positions(methyl_positions, step_size=10, overhang=30, nuc_length=110): #edited this to 110
    '''
    ...
    can modify how many and by how much a nucleosome has to overlap the molecule (currently 30 bp)
    '''
...
def nuc_overlaps_tf_binding(methyl_positions, state):
    ...
    for idx, p in enumerate(methyl_positions):
        for nuc in nuc_positions:
            if p >= (nuc[0]-space_bw_tf_and_nuc) and p <= (nuc[1]+space_bw_tf_and_nuc):
                protection[idx] = 1
        ...
        for tfbs in binding_sites:
            if p >= tfbs[0] and p <= tfbs[1]:
                if protection[idx] == 1: # checking to see if nucleosome overlaps a bound TF
                    return True
                if (idx > 0 and protection[idx-1] == 1) or (idx+1 < len(methyl_positions) and protection[idx+1] == 1): # also checking if a nucleosome overlaps the _flanks_ of a bound tf, this way a nucleosome cannot abut a bound tf
                    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument("--extra_motif_bps", dest="extra_motif_bps", type=int, default=0, help="A fixed bp offset compared to the position_list, making motifs larger with this number of bps, both to the left and right")
    ...
    with PdfPages(args.plot_file) as plots:
        ...
        pos_file = load_tfbs_positions(args.positions_file,args.extra_motif_bps)
