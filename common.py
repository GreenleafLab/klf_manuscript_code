...
def load_tfbs_positions(f,extra_motif_bps=0):
    '''
    ...
    with open(f) as pos_file:
        for line in pos_file:
            if line.startswith('>'):
                ...
                line_split[0] = int(line_split[0])-extra_motif_bps
                line_split[1] = int(line_split[1])+extra_motif_bps
                tup = (int(line_split[0]), int(line_split[1]), line_split[2])
                pos_dict[amplicon].append(line_split)

    return pos_dict

def add_fillbetween(ax, locs):
    '''
...
def cluster_single_reads(mat):
    ...
    return mat.iloc[rows]
    
def plot_single_reads(mat, ax, axis_label=None):
    '''
    ...
    ax.imshow(new_mat, aspect='auto', cmap=plt.get_cmap('gray'))
    if axis_label:
...
def plot_single_read(mat, idx, ax, fillbetween=None):
    '''
